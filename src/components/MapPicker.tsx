...
export default function MapPicker({ radius, onLocationSelect, onClose, initialLocation }: MapPickerProps) {
  ...
  const handleMapClick = async (latlng: LatLng) => {
    ...
    try {
      ...
      const addressText = locationInfo
        ? formatAddress(locationInfo)
    ...
    <div className='p-4 sm:p-6 space-y-4'>
      ...
      <div className='bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4'>
        <div className='flex items-start space-x-3'>
          ...
          <div>
            <h3 className='font-semibold text-blue-800 dark:text-blue-200 mb-1'>Wybierz lokalizację na mapie</h3>
            <p className='text-sm text-blue-700 dark:text-blue-300'>
              Kliknij na mapę aby wybrać punkt, wokół którego chcesz monitorować loty. Niebieski okrąg pokazuje obszar
              monitorowania ({radius} km).
            </p>
          </div>
        </div>
      </div>
      ...
      {selectedLocation && (
        <div className='bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-4 space-y-3'>
          ...
          {isLoadingAddress ? (
            ...
            <p className='text-sm text-green-700 dark:text-green-300 font-medium'>📍 {address}</p>
          )}
